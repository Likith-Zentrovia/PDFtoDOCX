...
class PDFExtractor:
    ...
    COLUMN_GAP_THRESHOLD = 15  # Minimum gap to consider as column separator
    HEADER_REGION_PCT = 0.08  # Top 8% of page (for filtering in column detection)
    FOOTER_REGION_PCT = 0.08  # Bottom 8% of page (for filtering in column detection)

    # Column detection parameters
    COLUMN_DETECTION_BINS = 100  # Fine-grained bins for accurate gutter detection
    MIN_GUTTER_WIDTH = 12  # Minimum width for a gutter between columns
    COLUMN_CONTENT_THRESHOLD = 3  # Minimum lines to consider a region as content
    MIN_COLUMN_WIDTH_RATIO = 0.20  # Minimum column width as ratio of page width
    ...
    def _extract_page(self, page_num: int) -> PageContent:
        ...
        column_info = self._detect_columns(text_lines, width, height)
    ...
    def _detect_columns(self, lines: List[TextLine], page_width: float, page_height: float = 792) -> ColumnInfo:
        """
        ...
        1. Filter out header/footer region lines (they often have different positions)
        2. Analyze horizontal text distribution with fine-grained bins
        3. Find gaps in the text coverage in the middle region of the page
        4. Validate gutters by checking for actual text gaps
        """
        if not lines:
            return ColumnInfo(
                num_columns=1,
        ...
        # Pass 1: Filter out header/footer regions for column detection
        # These regions often have different text positions that can confuse column detection
        ...
        content_lines = [line for line in lines
                         if line.bbox[1] > header_threshold and line.bbox[3] < footer_threshold]

        # If we filtered out too many lines, use all lines
        ...
        x_lefts = [line.bbox[0] for line in content_lines]
        x_rights = [line.bbox[2] for line in content_lines]

        content_left = min(x_lefts)
        content_right = max(x_rights)
        content_width = content_right - content_left

        if content_width <= page_width * 0.3:
            return ColumnInfo(
                num_columns=1,
        ...
        # Pass 2: Find potential gutters using gap analysis
        # Look for vertical gaps in the middle of the page where no text exists
        page_mid = page_width / 2
        search_start = page_mid - page_width * 0.25  # Search from 25% to 75% of page
        search_end = page_mid + page_width * 0.25

        # For each line, record its right edge (for left column) and left edge (for right column)
        left_column_rights = []  # Right edges of lines that are likely in left column
        right_column_lefts = []  # Left edges of lines that are likely in right column

        for line in content_lines:
            line_center = (line.bbox[0] + line.bbox[2]) / 2
            line_width = line.bbox[2] - line.bbox[0]

            # Skip very short lines (they might be page numbers, bullets, etc.)
            if line_width < content_width * 0.1:
                continue

            # If line is clearly in the left half of the page
            if line.bbox[2] < page_mid + 20:
                left_column_rights.append(line.bbox[2])
            # If line is clearly in the right half of the page
            elif line.bbox[0] > page_mid - 20:
                right_column_lefts.append(line.bbox[0])
            # Line spans the middle - likely single column or wide element
            # Don't count these for gutter detection

        # Pass 3: Calculate the gutter position
        gutter_position = None

        if left_column_rights and right_column_lefts:
            # We have lines on both sides of the midpoint
            max_right_of_left = max(left_column_rights)
            min_left_of_right = min(right_column_lefts)

            gap = min_left_of_right - max_right_of_left

            if gap >= self.MIN_GUTTER_WIDTH:
                # We found a clear gutter
                gutter_position = (max_right_of_left + min_left_of_right) / 2

        # Pass 4: Validate using bin analysis
        if gutter_position is None:
            # Try bin-based detection as fallback
            ...
            # Look for consecutive empty bins in the middle region
            mid_bin_start = int(search_start / bin_width)
            mid_bin_end = int(search_end / bin_width)
            ...
            while i < mid_bin_end:
                if bin_counts[i] == 0:
                    gutter_start = i
                    while i < mid_bin_end and bin_counts[i] == 0:
                        i += 1
                    gutter_end = i

                    gutter_width = (gutter_end - gutter_start) * bin_width

                    if gutter_width > best_gutter_width and gutter_width >= self.MIN_GUTTER_WIDTH:
                        best_gutter = (gutter_start + gutter_end) / 2 * bin_width
                        best_gutter_width = gutter_width
                else:
                    i += 1

            if best_gutter is not None:
                gutter_position = best_gutter

        # Pass 5: Final validation - make sure gutter doesn't split any lines
        if gutter_position is not None:
            # Check if any line crosses the gutter
            lines_crossing = 0
            for line in content_lines:
                # Use some margin to avoid edge cases
                line_left = line.bbox[0] + 5
                line_right = line.bbox[2] - 5
                if line_left < gutter_position < line_right:
                    lines_crossing += 1

            # If too many lines cross the gutter, it's probably not a valid gutter
            if lines_crossing > len(content_lines) * 0.1:  # More than 10% cross
                gutter_position = None
        ...
        left_boundary = (content_left, gutter_position)
        right_boundary = (gutter_position, content_right)

        # Validate column widths
        min_col_width = page_width * self.MIN_COLUMN_WIDTH_RATIO

        left_width = left_boundary[1] - left_boundary[0]
        right_width = right_boundary[1] - right_boundary[0]

        if left_width < min_col_width or right_width < min_col_width:
            return ColumnInfo(
                num_columns=1,
                boundaries=[(content_left, content_right)],
                gutter_positions=[]
        ...
        return ColumnInfo(
            ...
            gutter_positions=[gutter_position]
        )
    ...
    def _group_lines_into_blocks(
        ...
        for line in sorted_lines:
            ...
            if not start_new_block:
                # Check for italic/bold transition (often indicates section boundary)
                # If previous line was italic but current is not (going from italic to regular)
                if prev_line.is_italic and not line.is_italic:
                    # Italic to regular change = new block (common for [Recommendation] -> paragraph)
                    start_new_block = True
                # Also check if previous was bold+italic (like [Recommendation]) and current is regular
                elif prev_line.is_bold and prev_line.is_italic and (not line.is_bold or not line.is_italic):
                    start_new_block = True
            ...
            if not start_new_block:
                ...
                # Check if line starts significantly to the left of previous line
                # This often indicates returning to normal paragraph from indented bullet
                moved_left = line.bbox[0] < prev_line.bbox[0] - 5

                # Check if this is significantly indented or de-indented
                is_indented = x_diff > 15 and x_diff < col_width * 0.15
                is_significantly_different = x_diff > col_width * 0.15

                if is_significantly_different and vertical_gap > avg_font_size * 0.2:
                    # Significant X change = likely new block
                    start_new_block = True
                elif moved_left and vertical_gap > avg_font_size * 0.3:
                    # Line moves left (e.g., from indented bullet to paragraph)
                    start_new_block = True
                elif is_indented and vertical_gap > avg_font_size * 0.4:
                    # Indented with vertical gap = new paragraph
                    start_new_block = True

            if not start_new_block:
                # Check for special patterns that indicate section breaks
                line_text = line.text.strip()
                if line_text:
                    # Section number patterns (e.g., "1.2.1", "2.3")
                    if re.match(r'^\d+\.\d+', line_text):
                        start_new_block = True
                    # [Recommendation] pattern
                    elif line_text.startswith('[') and ']' in line_text:
                        start_new_block = True
